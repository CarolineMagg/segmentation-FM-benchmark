########################################################################################################################
# Inference script for SAM2 in video mode
...
#           model_type (default: sam2.1_hiera_base_plus)
...
from src.inference.sam2.sam2_utils import all_sam2_model_type_options, create_sam2_predictor, create_png_video_dir, \
    prompt_based_prediction_sam2_video_style_combination
...
def run_inference_sam2_video(json_file: Union[Path, str], output_folder: Union[Path, str], prompt_type: list[str],
                             ...
                             model_type: str = "sam2_new_hiera_base_plus",
    ...
    predictor = create_sam2_predictor(device, model_type, video_predictor=True)

    print("")
    print(f"SAM2 video inference with {model_type}")
    ...
    # meta data json
    all_meta_data = {"model": "SAM2 video", "model_type": model_type,
    ...
    for file_id in file_ids:
        ...
        for idx, label in enumerate(labels_values):
            ...
            for reverse, s, f_n in zip([False, True], start_frame_idx, max_frame_num_to_track):
                tmp = prompt_based_prediction_sam2_video_style_combination(predictor, inference_state,
...
if __name__ == '__main__':
    ...
    run_inference_sam2_video(json_file=args.json_file,
