########################################################################################################################
# Inference script for SAM3 in image mode
# prompts are given as 2D prompts (points or bounding boxes) stored in json file
# settings: prompt type, number of points, number of random points, model_type (default: sam2.1_hiera_base_plus)
# masks are stored one-hot-encoded (pixel value = label value), stored per class (since SAM2 has single-class outputs)
########################################################################################################################
import argparse
import json
import os
...
from src.inference.sam3.sam3_utils import create_sam3_predictor, extract_and_process_slice_like_sam3, \
    prompt_based_prediction_sam3_image_style_combination
...
def run_inference_sam3(json_file: Union[Path, str], output_folder: Union[Path, str], prompt_type: list[str],
    ...
    predictor, model = create_sam3_predictor(device, video_predictor=False)
    ...
    # meta data json
    all_meta_data = {"model": "SAM3 image",
    ...
    for file_id in file_ids:
        ...
        for slice_idx in tqdm(range(len(input_image)), desc="Slices"):
            ...
            if str(slice_idx) in all_slices_with_prompts:
                ...
                input_array: np.ndarray = extract_and_process_slice_like_sam3(input_image, slice_idx)
                inference_state = predictor.set_image(input_array)
                ...
                for idx, label in enumerate(labels_values):
                    ...
                    if str(slice_idx) in slices_with_prompts:
                        ...
                        if len(input_prompt_points) > 0 or len(input_prompt_bbox) > 0:
                            ...
                            preds_msk: Optional[np.ndarray] = prompt_based_prediction_sam3_image_style_combination(
...
if __name__ == '__main__':
    ...
    run_inference_sam3(json_file=args.json_file,
