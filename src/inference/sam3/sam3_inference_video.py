########################################################################################################################
# Inference script for SAM3 in video mode
# slice-by-slice inference with memory bank and propagation
...
#           multi class output (SAM2 supports multiple classes during inference -> but this also requires more memory)
...
from src.inference.sam3.sam3_utils import create_sam3_predictor, create_png_video_dir, \
    prompt_based_prediction_sam3_video_style_combination
...
def run_inference_sam3_video(json_file: Union[Path, str], output_folder: Union[Path, str], prompt_type: list[str],
                             ...
                             multi_class: bool = False, use_volume_limits: bool = False,
                             nnunet_suffix: bool = True, debug: bool = False, store_all: bool = True):
    ...
    predictor, model = create_sam3_predictor(device, video_predictor=True)

    print("")
    print(f"SAM3 video inference")
    ...
    if multi_class:
        raise RuntimeError("currently multi_class is not supported.")

    # meta data json
    all_meta_data = {"model": "SAM3 video",
                     ...
                     "gap_between_frames": gap_between_frames, "multi_class": multi_class,
                     "use_volume_limits": use_volume_limits}
    ...
    for file_id in file_ids:
        ...
        # create png files from slices
        video_dir = output_dir / "tmp_video_dir"
        create_png_video_dir(video_dir, input_image)
        ...
        for idx, label in enumerate(labels_values):
            ...
            for reverse, s, f_n in zip([False, True], start_frame_idx, max_frame_num_to_track):
                tmp = prompt_based_prediction_sam3_video_style_combination(predictor, inference_state,
                                                                           input_point_original, input_bbox_original,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="SAM3 inference for combination of prompts from json file")
    ...
    run_inference_sam3_video(json_file=args.json_file,
                             ...
                             multi_class=False,  # currently not supported
