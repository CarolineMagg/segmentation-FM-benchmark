########################################################################################################################
# Inference script for ScribblePrompt
# slice-by-slice inference of 128x128 input
# prompts are given as 2D prompts (points or bounding boxes) stored in json file
# settings: prompt type, number of points, number of random points, model_type (default: vit_b)
...
from src.inference.scribble_prompt.scribble_prompt_utils import create_scribble_prompt_predictor, \
    extract_and_process_slice_like_scribble_prompt, prompt_based_prediction_scribble_prompt_style_combination
...
from src.inference.utils_3dprompts import transform_2dbox_resize, transform_2dpoint_resize
from src.inference.utils_filehandling import read_image_depth_first, write_output_masks_to_nii


def run_inference_scribble_prompt(json_file: Union[Path, str], output_folder: Union[Path, str], prompt_type: list[str],
                                  number_prompts: int, number_random_prompts: int, model_type: str = "nunet",
    ...
    predictor = create_scribble_prompt_predictor(device, model_type)

    print("")
    print(f"ScribblePrompt inference with {model_type}")
    ...
    if len(prompts_to_be_used) > 1 and "bbox" in prompts_to_be_used and number_of_prompts > 1 and model_type == "sam":
        raise RuntimeError("bbox and point combination for multiple setting is not supported for sam")
    ...
    for file_id in file_ids:
        ...
        for slice_idx in tqdm(range(len(input_image)), desc="Slices"):
            ...
            if str(slice_idx) in all_slices_with_prompts:
                ...
                input_array: torch.Tensor = extract_and_process_slice_like_scribble_prompt(input_image, slice_idx,
                                                                                           device)
                ...
                for idx, label in enumerate(labels_values):
                    ...
                    if str(slice_idx) in slices_with_prompts:
                        ...
                        input_prompt_bbox = transform_2dbox_resize(input_prompt_bbox,
                                                                   input_image.shape, np.array([128, 128]))
                        input_prompt_points = transform_2dpoint_resize(input_prompt_points,
                                                                       input_image.shape, np.array([128, 128]))
                        # if prompts are not empty
                        if len(input_prompt_points) > 0 or len(input_prompt_bbox) > 0:
                            classes_used.append(label)
                            preds_msk: Optional[np.ndarray] = prompt_based_prediction_scribble_prompt_style_combination(
                                predictor, input_array, input_prompt_points, input_prompt_bbox, H, W)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="ScribblePrompt inference for combination of prompts from json file")
    ...
    parser.add_argument("--model_type", default="sam", choices=["unet", "sam"])
    args = parser.parse_args()

    run_inference_scribble_prompt(json_file=args.json_file,
